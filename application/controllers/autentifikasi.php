...
{ 
    ...
    public function index() 
    {
        ...
        if ($this->form_validation->run() == false) { 
            $data['judul'] = 'Login'; 
            $data['user'] = ''; 
            //kata 'login' merupakan nilai dari variabel judul dalam array 
            //$data dikirimkan ke view aute_header 
        ...
        if ($user) { 
            ...
            if ($user['is_active'] == 1) { 
                ...
                if (password_verify($password, $user['password'])) { 
                    ...
                    salah!!</div>'); 
                    redirect('autentifikasi'); //ini harus autentifikasi , nanti ganti
                }
            } else { 
                $this->session->set_flashdata('pesan', '<div 
